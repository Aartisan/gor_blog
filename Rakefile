...
namespace :main do
  ...
  task :preview do
    Rake::Task["main:clean_compile_file"].invoke
    system "gor compile && gor http"
  end
...
namespace :blog do
  ...
  task :page do
    name = ENV["title"] || "ray_new_blog.md"
    p name.to_s
  ...
  task :post do
    ...
    open(filename, 'w') do |post|
      ...
      post.puts "date: #{date}"
      post.puts 'description: ""'
      post.puts "permalink: '/#{Time.now.year}/#{Time.now.month}/#{Time.now.day}/#{title}'"
